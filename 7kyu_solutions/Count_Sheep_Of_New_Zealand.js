...
function lostSheep(friday,saturday,total){  
  ...
  let sat = null;
   ...
   if (friday.length > 0) { fri = friday.reduce((total, num) => {return total+num}) }
  if (saturday.length > 0) { sat = saturday.reduce((total, num) => {return total+num}) }
  
  return total - (fri + sat);
